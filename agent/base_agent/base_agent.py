...
class BaseAgent:
    ...
    def __init__(
        ...
        market: str = "us",
        verbose: bool = False,
        sentiment_mode: Optional[str] = "trend_following",
        ...
        Args:
            ...
            fixed_temperature: If set, overrides sentiment temperature with this value (0.0-1.0)
        """
    ...
    def _get_default_mcp_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "indicators": {
                "transport": "streamable_http",
                "url": f"http://localhost:{os.getenv('INDICATORS_HTTP_PORT', '8004')}/mcp",
            },
            "sentiment": {
                "transport": "streamable_http",
                "url": f"http://localhost:{os.getenv('SENTIMENT_HTTP_PORT', '8006')}/mcp",
            },
        }
    ...
    async def initialize(self) -> None:
        ...
        # Resolve LLM temperature from sentiment or fixed override
        self.current_temperature = None
        self.sentiment_data = None

        if self.fixed_temperature is not None:
            self.current_temperature = self.fixed_temperature
            print(f"🌡️  Using fixed temperature: {self.current_temperature}")
        elif self.sentiment_mode:
            try:
                use_contrarian = self.sentiment_mode.lower() == "contrarian"
                self.sentiment_data = get_sentiment_temperature(use_contrarian=use_contrarian)
                self.current_temperature = self.sentiment_data["temperature"]
                print(f"🌡️  Sentiment Temperature: {self.current_temperature}")
                print(f"   Ticker: {self.sentiment_data['ticker_used']} | "
                      f"P/C Ratio: {self.sentiment_data['put_call_ratio']} | "
                      f"Sentiment: {self.sentiment_data['sentiment_label']} | "
                      f"Mode: {self.sentiment_data['mode']} | "
                      f"Source: {self.sentiment_data['source']}")
            except Exception as e:
                print(f"⚠️  Could not fetch sentiment temperature: {e}. Using default.")

        try:
            # Build model kwargs — include temperature if available
            model_kwargs = {
            ...
            if self.current_temperature is not None:
                model_kwargs["temperature"] = self.current_temperature
            ...
            if "deepseek" in self.basemodel.lower():
                self.model = DeepSeekChatOpenAI(**model_kwargs)
            else:
    ...
    def _setup_logging(self, today_date: str) -> str:
        ...
        # Sanitize date string for Windows (replace colons with dashes)
        safe_date = today_date.replace(":", "-") if sys.platform == 'win32' else today_date
